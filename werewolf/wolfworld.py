...
class WolfWorld(World):
    ...
    def __init__(self, cli: Connection = None, csv_logger = None, txt_logger = None, wolf_strategy="window", village_strategy="window", seed=1234, listener=None):
        ...
        self.day_room = load_room("werewolf/tavern.json")
        self.night_room = load_room("werewolf/cave.json")
    ...
    def reset_votes(self):
        ...
        if len(self.voters) % 2 == 0:
            self.tiebreaker_needed = True
            self.votes_needed = len(self.voters) / 2
        else:
            self.tiebreaker_needed = False
            self.votes_needed = len(self.voters) / 2 + 1
