...
class World(Process, ABC):
    ...
    def resolve_majority_vote(self, tiebreaker = False) -> str | None:
        ...
        if most_common[0][1] >= self.votes_needed and not self.tiebreaker_needed:
            return most_common[0][0] # odd voter total, reached goal
        elif most_common[0][1] > self.votes_needed and self.tiebreaker_needed:
            return most_common[0][0] # even voter total, exceeded goal
        elif len(most_common) == 2 and most_common[0][1] == most_common[1][1] and self.tiebreaker_needed and tiebreaker:
            if most_common[0][0] == None:
                return most_common[0][1]
            elif most_common[0][1] == None:
                return most_common[0][0]
            else:
                return random.choice([most_common[0][0], most_common[1][0]]) # even voter total, two at goal, random
        else:
            return None # majority not reached
    ...
    def reset_votes(self):
        ...
        with self.actors_lock:
            if len(self.actors) % 2 == 0:
                self.tiebreaker_needed == True
                self.votes_needed = len(self.actors) / 2
            else:
                self.tiebreader_needed == False
                self.votes_needed = len(self.actors) / 2 + 1
